...
| Print Rotation | `printRotation` 纯函数 + `PrintReviewDialog` / `PrintPages` | 本合同 | 纸张与内容 `0/90/180/270`，字号固定 | 纯函数 + 打印 DOM + 浏览器 |
...
| Font size picker | 已提交字号 | 悬停选项并临时更新文字与边框 | 方向键移动高亮并临时预览 | 点击或 Enter 提交 | n/a | n/a | Escape、Tab 或失焦关闭并恢复已提交字号 |
...
| Print rotation | 文字默认 0°、纸张保持预设方向 | 旋转按钮边线加深 | 可见焦点 | 每次顺时针 90°，270° 后回到 0° | 图片不显示旋转按钮；90°/270° 时纸张宽高同步交换，字号不缩小 | n/a | 继续旋转回 0° |
...
| Rotate print text | 打印检查中“旋转 90°” | 纸张、打印区域和全部文字围绕中心整体旋转 | 同一对话框 | 缩略图与打印 DOM 同步按 `0/90/180/270` 循环；逐行坐标和字号不变 | 90°/270° 同步交换打印页宽高；关闭对话框清零 | 保持旋转按钮焦点 | 用户确认（2026-09-05） |
| Print | “打印这一组” | 显示程序生成张数，并提示系统打印份数保持 1 | 系统打印窗口 | 显示 `1 × 程序生成张数 = 实际打印张数`；打印页使用临时纸张角度，字号保持 100% | 取消系统打印后保留当前草稿 | 返回打印检查触发按钮 | 用户确认需求 |
...
- Physical preview contract: 纸张宽高使用毫米，尺寸工具只保留可编辑的宽度与高度；内容区域移动/缩放后始终限制在纸张内，并由屏幕预览和打印页共同消费。首次打开自动创建空白唛头，空白预览可直接输入多行文字。点击“打印这一组”后记住该组宽高，之后从任一入口新增唛头均默认使用该宽高；只编辑宽高不更新这项默认值，清空草稿也保留最近打印宽高。全部字号可选择自动适配或固定物理字号；悬停或方向键高亮只临时预览字号并同步改变文字边框，点击或 Enter 才提交。固定字号和逐行/局部字号覆盖都是上限：越界时只向下缩小到可完整显示的最大字号，未越界不放大；屏幕预览与打印页消费同一逐行字号映射。最小字号仍越界或文字行重叠时阻止打印。打印检查循环旋转整张纸、打印区域和全部文字，逐行坐标及字号保持不变；90°/270° 时打印页同步交换毫米宽高，不进行旋转缩放，关闭对话框清除所有临时角度。打印页排除编辑边框、控制点和临时字号预览。
