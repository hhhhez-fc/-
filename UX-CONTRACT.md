...
| Print rotation | 文字默认 0°、纸张宽高固定 | 旋转按钮边线加深 | 可见焦点 | 每次顺时针 90°，270° 后回到 0° | 图片不显示旋转按钮；90°/270° 时整体等比缩小 | n/a | 继续旋转回 0° |
...
| Rotate print text | 打印检查中“旋转 90°” | 围绕内容区中心整体旋转，同步应用统一缩放 | 同一对话框 | 缩略图与打印 DOM 同步按 `0/90/180/270` 循环；逐行坐标和纸张宽高不变 | 90°/270° 按内容区短长边比例缩小；关闭对话框清零 | 保持旋转按钮焦点 | 用户确认（2026-09-05） |
| Print | “打印这一组” | 显示程序生成张数，并提示系统打印份数保持 1 | 系统打印窗口 | 显示 `1 × 程序生成张数 = 实际打印张数`；文字使用临时角度，纸张宽高不旋转 | 取消系统打印后保留当前草稿 | 返回打印检查触发按钮 | 用户确认需求 |
