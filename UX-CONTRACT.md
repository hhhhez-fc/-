...
- Responsive transformation: 桌面工作区的三个板块以 300 / 540 / 340px 默认宽度保持同一行，顺序和宽高保存在本机；放大后的总宽度超过视口时横向滚动而不换行。拖动标题时整块随指针移动，按最近水平插槽预览，松手后吸附换位；边缘只负责调宽高，最小宽度 180px。预览板块内的全部文字工具栏固定在标题和画布之间，默认两行；板块窄于 500px 时四个下拉折为两列。720px 以下工作区按逻辑顺序单栏堆叠并取消横向溢出，标题恢复纵向触摸滚动，禁用指针换位与横向调宽，但保留标题左右方向键换位和纵向调高。“使用过的唛头”始终位于录入来源内部，不参与独立排序。
