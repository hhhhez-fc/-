...
- Physical preview contract: 纸张宽高使用毫米，尺寸工具只保留可编辑的宽度与高度；内容区域移动/缩放后始终限制在纸张内，并由屏幕预览和打印页共同消费。首次打开自动创建空白唛头，空白预览可直接输入多行文字。全部字号可选择自动适配或固定物理字号；当前行文字边框缩放会写入该行字号。越界或文字行重叠时保持字号并显示校验错误；打印页排除编辑边框与控制点。
