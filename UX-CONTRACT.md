...
| Print rotation | 文字默认 0°、纸张宽高固定 | 旋转按钮边线加深 | 可见焦点 | 每次顺时针 90°，270° 后回到 0° | 图片不显示旋转按钮；溢出组禁用打印 | n/a | 旋转后溢出显示原因 |
...
| Rotate print text | 打印检查中“旋转 90°” | 即时重算旋转包围盒 | 同一对话框 | 缩略图与打印 DOM 同步按 `0/90/180/270` 循环 | 旋转后溢出则禁用该尺寸组；关闭对话框清零 | 保持旋转按钮焦点 | 用户浏览器批注确认 |
