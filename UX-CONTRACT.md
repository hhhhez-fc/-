...
| Print rotation | 文字默认 0°、纸张保持预设方向和原始毫米宽高 | 旋转按钮边线加深 | 可见焦点 | 每次顺时针 90°，270° 后回到 0° | 图片不显示旋转按钮；只逐行旋转文字，坐标和字号不变 | n/a | 继续旋转回 0° |
...
| Rotate print text | 打印检查中“旋转 90°” | 每行文字围绕自身锚点旋转，纸张和打印区域不旋转 | 同一对话框 | 缩略图与打印 DOM 同步按 `0/90/180/270` 循环；逐行坐标、字号和纸张宽高不变 | 图片不旋转；关闭对话框清零 | 保持旋转按钮焦点 | 用户确认（2026-09-05） |
