...
| Draggable text | 每行细虚线边框 | 抓取光标、边框实线 | 已选行蓝色实线；活动行四角控制点、方向键微调 | 单击累加；整组拖动/方向键移动保持相对间距且不改变固定字号 | n/a | n/a | 固定字号越界时保留字号并阻止打印；自动适配才缩小 |
