...
| Workspace panel | 三板块网格；历史并入录入来源 | 标题和边缘提示可拖动 | 标题方向键换位；边缘方向键调宽高 | 拖到目标左右半区后吸附换位 | 窄屏禁用指针换位和横向调宽，保留方向键换位 | n/a | 非法尺寸由布局域钳制 |
...
| Arrange workspace panels | 拖动板块标题到目标左/右半区，或聚焦标题后按左右方向键 | 即时本地更新 | 吸附到目标前/后位置 | 布局随草稿保存在本机 | 恢复默认布局 | 保持标题焦点 | 用户浏览器批注确认 |
...
- Responsive transformation: 桌面工作区的三个板块顺序和宽高保存在本机；拖动标题到目标左右半区时吸附换位，边缘只负责调宽高。720px 以下按逻辑顺序单栏堆叠，标题恢复纵向触摸滚动，禁用指针换位与横向调宽，但保留标题左右方向键换位和纵向调高。“使用过的唛头”始终位于录入来源内部，不参与独立排序。
