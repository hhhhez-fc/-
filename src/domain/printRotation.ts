...
export function getPrintRotationScale(width: number, height: number, rotation: PrintRotation): number {
  if (rotation === 0 || rotation === 180 || width <= 0 || height <= 0) return 1;
  return Math.min(width / height, height / width);
}

export function contentRotationTransform(width: number, height: number, rotation: PrintRotation): string {
  const scale = Number(getPrintRotationScale(width, height, rotation).toFixed(6));
