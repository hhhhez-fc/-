...
工作区由录入、预览、校对和使用记录四个可重排板块组成。操作员可拖动标题调整顺序、拖动边缘调整宽高，并以 75%–125% 独立缩放板块内容；布局与草稿一起保存在当前浏览器，同时提供缩小、放大和恢复默认布局。低频的常用尺寸、精确尺寸与打印区域数值设置收进原生详情折叠区，全部文字样式和自动排列保持直接可见。720px 以下按逻辑顺序单栏堆叠，禁用无效的换位与横向调宽，但保留纵向调高和缩放。
