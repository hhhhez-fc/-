...
大于 1120px 时使用“录入 220px 起 / 中央预览 540px 起 / 校对 250px 起”的三栏工作台，把最终纸张预览放在视觉中心并作为最大主操作面；预览画布至少约 38rem 高，宽屏纸张可扩展到 54rem。低频的常用尺寸、精确尺寸与打印区域数值设置收进原生详情折叠区，全部文字样式和自动排列保持直接可见。页面本身保持自然滚动，列表和预览在足够高度时可拥有内部滚动并启用稳定滚动条槽。窄屏按录入、列表、预览顺序堆叠，打印动作不遮挡字段。
...
预览纸内的蓝色裁切框表示实际内容打印区域：拖动框内空白处或图片可整体移动区域，八个控制点调整边界，毫米数字字段和方向键提供等价的非拖动操作；区域始终限制在纸张内。每一行文字仍是独立直接操作对象，并持续显示细虚线文字边框；当前行使用蓝色实线和四角控制点。按下或点击只选中当前行并保持原位，指针移动超过 4px 后才进入拖动；拖动从原位置计算位移，靠近九宫格锚点自动吸附。拖动文字边框四角时按比例改变字号，方向键提供等价字号调整，Shift 加速；双击某行可在原位置直接编辑，输入态不显示边框或底色，只保留文字和插入光标。每行可单独设置字体、字号、强调和横排/竖排；字符选区仍可覆盖该行的局部样式，控件变化立即生效，不再需要二次“应用”。右侧“全部文字样式”会明确覆盖已有逐行与局部字符的同类属性；“全部字号”直接设置固定物理字号，不再暴露字号模式。自动排列先把非空行上下均匀分布，再按下拉选择左对齐、水平居中、右对齐或保持当前左右位置；单行放在垂直中央。移动、排列或缩放造成越界及相邻行重叠时保持字号并显示校验错误。预览把毫米、pt 和边框统一缩放到屏幕纸张，打印消费同一物理字号与区域坐标，但不输出编辑边框、控制点或辅助线。
