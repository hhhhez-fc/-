...
本次交互约定：录入、预览、校对三个板块都可收起为 52px 边栏并恢复原有宽高，收起状态随草稿保存在本机；窄屏时边栏改为水平形态。预览工具栏的尺寸区始终显示且只保留可编辑的宽度、高度，不再使用折叠按钮、尺寸预设或打印区域数字字段。

工作区由录入、预览和校对三个可重排板块组成，“使用过的唛头”作为紧凑区域并入录入来源。桌面默认宽度为 300 / 540 / 340px，三个板块始终保持同一行；每块可收起为 52px 边栏并恢复原有宽高，收起状态与草稿一起保存在当前浏览器。用户放大到超过视口后由工作区横向滚动，不把板块折到下一行。操作员拖动标题时整块板块随指针抬起，工作区按指针的水平位置显示最近插槽；横向溢出时靠近工作区左右边缘会持续滚动，松手后才吸附换位。标题获得焦点后可用左右方向键完成同样的换位。宽高只通过右边缘、下边缘或右下角拖拽调整，最小宽度 180px，并提供恢复默认布局。预览板块先显示标题，再显示紧凑文字工具栏及可直接修改的宽度、高度，最后显示画布。720px 以下按逻辑顺序单栏堆叠，收起状态显示为水平边栏，取消横向溢出。
...
首次打开或清空草稿后自动建立一条空白唛头。空白文字唛头在预览裁切框内显示多行输入框，可直接录入；输入即时同步正文，并按行生成可独立移动与设置样式的文字对象。
...
预览纸内的蓝色裁切框表示实际内容打印区域：拖动框内空白处或图片可整体移动区域，八个控制点调整边界，方向键提供等价的非拖动操作；区域始终限制在纸张内。空白唛头在裁切框中显示多行输入框，录入后自动拆为独立文字行。每一行文字持续显示细虚线文字边框；当前行使用蓝色实线和四角控制点。按下或点击只选中当前行并保持原位，指针移动超过 4px 后才进入拖动；拖动从原位置计算位移，靠近九宫格锚点自动吸附。拖动文字边框四角时按比例改变字号，方向键提供等价字号调整，Shift 加速；双击某行可在原位置直接编辑。每行可单独设置字体、字号、强调和横排/竖排；字符选区仍可覆盖该行的局部样式。选择自动排列方式后立即把非空行上下均匀分布并应用左、中、右或保留横向位置。移动、排列或缩放造成越界及相邻行重叠时保持字号并显示校验错误。预览把毫米、pt 和边框统一缩放到屏幕纸张，打印消费同一物理字号与区域坐标，但不输出编辑边框、控制点或辅助线。
