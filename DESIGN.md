...
工作区由录入、预览和校对三个可重排板块组成，“使用过的唛头”作为紧凑区域并入录入来源。桌面默认宽度为 300 / 540 / 340px，三个板块始终保持同一行；用户放大到超过视口后由工作区横向滚动，不把板块折到下一行。操作员拖动标题时整块板块随指针抬起，工作区按指针的水平位置显示最近插槽；横向溢出时靠近工作区左右边缘会持续滚动，松手后才吸附换位。标题获得焦点后可用左右方向键完成同样的换位。宽高只通过右边缘、下边缘或右下角拖拽调整，最小宽度 180px，不再显示额外的移动或缩放按钮；布局与草稿一起保存在当前浏览器，并提供恢复默认布局。预览板块先显示标题，再显示紧凑文字工具栏，最后显示画布，让常用修改无需滚过整张预览。低频的常用尺寸、精确尺寸与打印区域数值设置收进工具栏第二行的原生详情折叠区。720px 以下按逻辑顺序单栏堆叠，取消横向溢出，禁用可能妨碍纵向滚动的指针换位与横向调宽，但保留标题方向键换位和纵向调高。
...
原生 `select` 是项目的选择器标准，接受 Windows 平台弹层外观。常规输入、选择器和按钮共享 40px 高度；预览标题下的全部文字样式采用类似 Excel 的两行紧凑工具栏，第一行放字体、字号、行距和自动排列下拉，第二行保留粗体、斜体、下划线，并把尺寸与打印区域收为折叠按钮。工具栏选择后直接更新下方预览；窄板块将四个下拉折为两列。文件区同时提供可见按钮和拖放区域。确认框为模态 `alertdialog`，支持焦点约束、Escape、取消初始焦点和触发器焦点恢复。
