...
工作区由录入、预览和校对三个可重排板块组成，“使用过的唛头”作为紧凑区域并入录入来源。操作员拖动标题到目标板块左、右半区时，板块吸附到目标前、后位置；标题获得焦点后可用左右方向键完成同样的换位。宽高只通过右边缘、下边缘或右下角拖拽调整，不再显示额外的移动或缩放按钮；布局与草稿一起保存在当前浏览器，并提供恢复默认布局。低频的常用尺寸、精确尺寸与打印区域数值设置收进原生详情折叠区，全部文字样式和自动排列保持直接可见。720px 以下按逻辑顺序单栏堆叠，禁用可能妨碍纵向滚动的指针换位与横向调宽，但保留标题方向键换位和纵向调高。
